...
As a user running `rcdir /W` on a directory where most filenames are short but a few are very long, I want the output to use variable-width columns so that short names don't waste horizontal space, allowing more columns and more filenames visible per screen.
...
**Independent Test**: Can be fully tested by running `rcdir /W` on a directory with mixed-length filenames (e.g., a directory containing both short names like `docs` and long names like `Microsoft.WindowsPackageManagerManifestCreator_8wekyb3d8bbwe`) and verifying the output uses more columns than the current uniform-width layout.

**Acceptance Scenarios**:

1. **Given** a directory with filenames of varying lengths and a 120-character-wide terminal, **When** the user runs `rcdir /W`, **Then** short filenames are displayed in narrower columns and long filenames in wider columns, fitting more entries per row than uniform-width would allow.
2. **Given** a directory where one filename is 63 characters and the rest are under 30 characters in a 126-character terminal, **When** the user runs `rcdir /W`, **Then** the output displays at least 2 columns (instead of the current single-column result).
3. **Given** a directory where all filenames are roughly the same length, **When** the user runs `rcdir /W`, **Then** the output is visually identical to the current behavior (all columns equal width).
...
**Independent Test**: Can be verified by running `rcdir /W` and confirming that entries fill columns top-to-bottom before moving to the next column.

**Acceptance Scenarios**:

1. **Given** a directory with 10 entries and a layout that fits 3 columns, **When** the user runs `rcdir /W`, **Then** entries 1–4 appear in column 1, entries 5–7 in column 2, and entries 8–10 in column 3 (top-to-bottom fill).
...
**Independent Test**: Can be tested by running `rcdir /W` with icons enabled (`/I`) and verifying that icon-bearing entries display without truncation or misalignment.

**Acceptance Scenarios**:

1. **Given** icons are enabled and a directory has entries with icons, **When** the user runs `rcdir /W`, **Then** each column's width accounts for the icon width (+2 characters) on a per-entry basis, not globally.
2. **Given** cloud status is enabled and some entries have cloud status indicators, **When** the user runs `rcdir /W`, **Then** each column's width accounts for the cloud status indicator (+2 characters) only for entries that display it.
...
- **FR-002**: The system MUST try candidate column counts from the maximum feasible down to 2, selecting the highest count whose total width fits within the console width. If no multi-column layout fits, a hardcoded single-column fallback is used.
...
- **FR-004**: The system MUST compute each column's required width using per-entry display width, which includes directory brackets (`[name]` when icons are off, +2), icon glyphs (+2 when icons are active and not suppressed for that entry), and cloud status indicators (+3 in non-icon mode, +4 in icon mode) as applicable.
