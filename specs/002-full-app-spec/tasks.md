...
- [X] T098 [P] Create output parity integration test framework — build both rcdir and tcdir, capture output for representative scenarios, compare line-by-line — in tests/output_parity/mod.rs
- [X] T099 [P] Add output parity test cases — single dir, recursive, sorted (/o:s, /o:-d), filtered (/a:d, /a:-hs), wide, bare, combined switches — in tests/output_parity/
...
- [X] T102 Final review — verify byte-identical output for representative test cases against TCDir, check column alignment, number formatting, color accuracy
