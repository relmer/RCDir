...
**Created**: 2026-03-25
**Status**: Draft
**Input**: User description: "Interactive PowerShell alias configuration: rcdir --set-aliases, --get-aliases, --remove-aliases with TUI for configuring root alias, sub-aliases, and profile storage location"

## Clarifications

### Session 2026-03-25

- Q: What should happen when the user's profile already contains manually-written rcdir aliases (no marker comments)? → A: Ignore them entirely — only manage marker-delimited blocks
- Q: What should happen when the terminal does not support ANSI VT escape sequences? → A: Not applicable — VT support is a baseline requirement for rcdir; no fallback needed
- Q: What should happen when marker comments exist but content between them was manually altered? → A: Replace the entire marker-delimited block unconditionally; the timestamped `.bak` backup protects the user. Block delimiters are required (multi-line functions). Marker comments must explicitly warn that rcdir manages and replaces the block.
- Q: How should the generated root function resolve the rcdir executable? → A: At setup time, check the path of the running rcdir.exe instance. If that path is on PATH, generate functions using the short name `rcdir`. If not on PATH, bake the full path into the generated functions.
- Q: Should the tool support Windows PowerShell 5.1 profile paths in addition to PowerShell 7+? → A: Yes, fully support both. Auto-detect which PS version launched rcdir by inspecting the parent process image name (`pwsh.exe` = 7+, `powershell.exe` = 5.1). `PSModulePath` env var is NOT usable — PS 7+ includes both `\PowerShell\` and `\WindowsPowerShell\` paths. All three commands (`--set-aliases`, `--get-aliases`, `--remove-aliases`) scope to the detected PS version's profile paths only.

### Session 2026-03-28

- Q: Should rcdir be aware of TCDir alias blocks in the same profile (this spec was ported from TCDir, and both tools write marker-delimited blocks)? → A: Fully independent — rcdir only scans for and manages `RCDir Aliases` markers, completely ignoring any TCDir blocks. No cross-tool coupling.
- Q: Should there be a retention/cleanup policy for timestamped `.bak` backup files created by FR-070? → A: No automatic cleanup — backups accumulate and are user-managed. This is the simplest and safest approach; users can delete old `.bak` files at their discretion.
- Q: If multiple rcdir alias blocks are found in the same profile file (e.g., from manual copy-paste), how should the scanner behave? → A: Process only the first block found. The scanner stops after finding the first opening/closing marker pair. Users with corrupted profiles can manually clean up duplicates.

## User Scenarios & Testing *(mandatory)*

### User Story 1 - First-Time Alias Setup (Priority: P1)

A user installs rcdir via winget and wants to set up short aliases so they can type `d` instead of `rcdir`, and `dd` for directory-only listings, `ds` for recursive search, etc. They run `rcdir --set-aliases` and are guided through an interactive menu to choose their root alias, select which sub-aliases to enable, pick where to save them, and optionally preview the changes before applying.

**Why this priority**: This is the core value proposition — making rcdir accessible through short, memorable commands. Without this, users must always type the full `rcdir` command.

**Independent Test**: Can be fully tested by running `rcdir --set-aliases` on a machine with no existing aliases. The user completes the wizard, a new alias block appears in their chosen profile, and after reloading the profile the aliases work.

**Acceptance Scenarios**:

1. **Given** rcdir is installed and no aliases exist, **When** the user runs `rcdir --set-aliases`, **Then** an interactive TUI launches showing step-by-step configuration screens.
...
A user wants to see what rcdir aliases are currently configured — both in their profile files and in the running session. They run `rcdir --get-aliases` and see a summary.
...
**Independent Test**: Can be tested by setting up aliases in a profile, running `rcdir --get-aliases`, and verifying the output shows the profile location, version, and actual function definitions.

**Acceptance Scenarios**:

1. **Given** aliases are stored in a profile file with rcdir marker comments, **When** the user runs `rcdir --get-aliases`, **Then** the output shows the profile variable name and path on one line, the generating version, and the actual `function xxx { ... }` lines from the alias block.
2. **Given** no rcdir aliases exist anywhere, **When** the user runs `rcdir --get-aliases`, **Then** the output says no rcdir aliases were found and suggests running `--set-aliases`.
3. **Given** aliases exist in multiple profile files, **When** the user runs `rcdir --get-aliases`, **Then** each profile location is shown with its function definitions, separated by 2 blank lines between profiles.

---

### User Story 3 - Update Existing Aliases (Priority: P2)

A user already has aliases configured with root `d` but now wants to change the root to `tc`, or wants to enable a sub-alias they previously skipped. They run `rcdir --set-aliases` and the system detects the existing configuration and offers to replace it.
...
1. **Given** rcdir aliases already exist in a profile with root `d`, **When** the user runs `rcdir --set-aliases`, **Then** the system detects the existing aliases and shows them, offering to update or replace.
...
A user wants to remove rcdir aliases from their profile and/or current session. They run `rcdir --remove-aliases` and the system scans for existing aliases, shows where they were found, and lets the user choose which locations to clean up.
...
1. **Given** rcdir aliases exist in one or more profile files, **When** the user runs `rcdir --remove-aliases`, **Then** only profiles containing rcdir aliases are listed as checkbox options, defaulting to unchecked (opt-in removal).
2. **Given** the user selects one or more profile files for removal, **When** they confirm, **Then** the rcdir alias block (between marker comments) is removed from each selected profile and no other profile content is modified.
3. **Given** no rcdir aliases are found anywhere, **When** the user runs `rcdir --remove-aliases`, **Then** the output says no rcdir aliases were found.
...
**Independent Test**: Can be tested by running `rcdir --set-aliases --whatif`, completing the wizard, and verifying no files were created or modified on disk while the console output shows the exact content that would be written.

**Acceptance Scenarios**:

1. **Given** the user runs `rcdir --set-aliases --whatif`, **When** the wizard completes, **Then** the console displays the profile path, the alias block content, and a message confirming no changes were made.
2. **Given** the user runs `rcdir --remove-aliases --whatif`, **When** they select a profile, **Then** the console shows which lines would be removed and confirms no changes were made.
...
- What happens when the user's profile already contains rcdir-related code that was manually written (not via `--set-aliases`, so no marker comments)? → **Resolved**: The tool ignores them entirely. Only marker-delimited blocks (per FR-040) are managed. Manual aliases are invisible to the tool.
- What happens when the user's terminal does not support ANSI escape sequences (very old conhost without VT mode)? → **Resolved**: Not applicable. VT support is a baseline requirement for rcdir; the alias wizard inherits this requirement.
...
- What happens if a profile file contains both TCDir and RCDir alias blocks? → **Resolved**: Each tool manages only its own marker-delimited blocks. RCDir ignores TCDir blocks entirely; they coexist independently.
- What happens if multiple RCDir alias blocks exist in the same profile file? → **Resolved**: The scanner processes only the first block found (first opening/closing marker pair). Duplicate blocks from manual copy-paste are the user's responsibility to clean up.
- What happens to `.bak` backup files over time — do they accumulate? → **Resolved**: No automatic cleanup. Timestamped backups accumulate and are user-managed. Users can delete old `.bak` files at their discretion.
...
- **FR-001**: rcdir MUST accept `--set-aliases` to launch the interactive alias configuration wizard
- **FR-002**: rcdir MUST accept `--get-aliases` to display all currently configured rcdir aliases and their source locations
- **FR-003**: rcdir MUST accept `--remove-aliases` to launch the interactive alias removal wizard
- **FR-004**: rcdir MUST accept `--whatif` as a modifier to `--set-aliases` and `--remove-aliases` to preview changes without applying them
...
- **FR-017**: The wizard title line (e.g., "RCDir Alias Setup") MUST be displayed in InformationHighlight (white). All `--whatif` status messages and indicators MUST be displayed in Error color (red).
...
- **FR-030**: If existing rcdir aliases are detected in any profile (via marker comments), the wizard MUST inform the user and offer to replace them

#### Alias Block Format

- **FR-040**: The persisted alias block MUST be wrapped in an 80-character-wide `#` banner. The header banner includes the version, a DO NOT EDIT warning, and usage instructions. The footer banner contains `# End RCDir Aliases`. The scanner identifies the block by looking for `#  RCDir Aliases` in the header and `# End RCDir Aliases` in the footer. See alias-block format example in quickstart.md. The scanner MUST ignore any TCDir alias blocks (`TCDir Aliases` markers) — each tool manages only its own blocks. If multiple RCDir alias blocks exist in the same file, only the first block found is processed.
- **FR-041**: At setup time, the tool MUST determine the path of the currently running rcdir.exe. If that path is reachable via PATH, the root alias function MUST invoke `rcdir` by short name. If not on PATH, the root alias function MUST invoke the full resolved path to rcdir.exe.
- **FR-042**: The root alias MUST be defined as a simple passthrough PowerShell function that invokes rcdir (by name or full path per FR-041) and passes all arguments via `@args`. No fallback behavior (e.g., `Get-ChildItem`) or post-invocation output (e.g., `Write-Host`)
...
- **FR-044**: The alias block header MUST include the rcdir version that generated it (e.g., `rcdir v5.2.1150`). The minor version bumps to 5.2 for this feature.

#### Remove Aliases Flow

- **FR-050**: The remove wizard MUST scan all PowerShell profile file paths for the detected PS version for rcdir marker comments before presenting options
- **FR-051**: The remove wizard MUST only list profile locations that contain rcdir aliases
- **FR-052**: Under each profile location, the remove wizard MUST display the specific alias names found
- **FR-053**: If no rcdir aliases are found in any location, the tool MUST display a message stating so and exit
...
- **FR-060**: `--get-aliases` MUST scan all PowerShell profile paths for the detected PS version for rcdir alias blocks
- **FR-061**: For each location containing aliases, the output MUST show the profile variable name and resolved path on one line in the format `$PROFILE.Xxx  (C:\path\to\file.ps1)` (matching the display format used in `--set-aliases` and `--remove-aliases` prompts), followed by a "Generated by rcdir vX.Y.Z" line, then the actual `function xxx { ... }` lines from the alias block (indented 4 spaces). Output spacing: 1 blank line after the command, 1 blank line between header and function lines, 1 blank line after the last function line, 2 blank lines between profile dumps when multiple profiles have aliases
- **FR-062**: `--get-aliases` MUST NOT require interactive input — it runs and exits

#### File Operations Safety

- **FR-070**: Before modifying any profile file, the tool MUST create a timestamped backup copy named `{filename}.{YYYY-MM-DD-HH-MM-SS}.bak`. No automatic cleanup of old backups is performed — retention is user-managed.
...
- **Root Alias**: The primary short command (e.g., `d`, `tc`) that maps to invoking rcdir with passthrough arguments
- **Sub-Alias**: A derived command that prepends specific rcdir flags before the user's arguments (e.g., `dd` → `d -a:d`)
...
- The winget portable install places `rcdir.exe` in a directory already on `PATH` via the winget links mechanism, so the alias function can reference `rcdir` by name rather than full path.
...
- **SC-001**: A new user can go from `winget install relmer.RCDir` to having working aliases in under 2 minutes
