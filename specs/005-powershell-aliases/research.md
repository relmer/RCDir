...
**Required Headers**: `<tlhelp32.h>` (not currently in Cargo.toml)
...
**Memory Management**: The `windows` crate handles `CoTaskMemFree` automatically via `PWSTR` wrapper types.

**Required Cargo Features**: `Win32_UI_Shell` in the `windows` crate
...
**Rationale**: RCDir already uses `SetConsoleMode` for VT output. Extending to raw input mode is natural. `ReadConsoleInputW` provides low-level keyboard events for arrow keys, space, enter, escape.

**Alternatives Considered**:
- `_getch()` / `_getwch()` — C runtime, not available in Rust
- Third-party TUI crate (e.g., `crossterm`, `ratatui`) — adds significant dependencies; direct Windows Console API via `windows` crate is simpler for this use case
...
**Existing Infrastructure**: The `Console` struct already has `GetConsoleMode`/`SetConsoleMode` and screen buffer info. TUI input can build on same handle management.
...
**Rationale**: `GetModuleFileNameW` reliably returns the running exe's full path. `SearchPathW(None, w!("rcdir.exe"), ...)` checks if the same name is findable via PATH without manual string parsing.

**Alternatives Considered**:
- Parsing `PATH` env var manually and comparing directories — fragile, case-sensitivity edge cases
- `where.exe rcdir.exe` via CreateProcess — process overhead, unnecessary

**Logic**:
1. `GetModuleFileNameW(nullptr, szExePath, MAX_PATH)` → full path of running rcdir.exe
2. `SearchPathW(nullptr, L"rcdir.exe", nullptr, MAX_PATH, szFound, nullptr)` → if found, compare resolved path
3. If `szFound` matches `szExePath`: rcdir is on PATH → generate `function d { rcdir @args }`
4. If not on PATH: generate `function d { & "C:\full\path\to\rcdir.exe" @args }`

---

## R5: Profile File I/O Patterns

**Decision**: Read entire file into memory, manipulate as `Vec<String>` of lines, write back atomically
...
- Opening scanner: line containing `#  RCDir Aliases`
- Closing scanner: line containing `#  End RCDir Aliases`
...
### Cargo.toml: Windows Crate Features to Add
```toml
[dependencies.windows]
features = [
    "Win32_System_Diagnostics_ToolHelp",  # ToolHelp process snapshot (R1)
    "Win32_UI_Shell",                      # SHGetKnownFolderPath, FOLDERID_* (R2)
]
```

### Existing Infrastructure to Leverage
- `Console` struct — output buffering, ANSI escape sequences, console mode management
- `CommandLine` struct — switch parsing framework (long switch table, validation)
- `Result<T, E>` with `?` operator — for all fallible Windows API calls
